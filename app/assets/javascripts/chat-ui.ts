...
export const chatListener = {
  onAnyEvent: function (evt: {
    ...
    if (evt.rule) {
      let systemMessageBanner: HTMLElement | null = document.getElementById(
        "systemMessageBanner",
      );
...
export function hookWindow(
  ...
  reactiveChatController: ReactiveChatController,
  proactiveChatController: ProactiveChatController,
) {
  w.InqRegistry = {
    listeners: [chatListener],
  };

  w.nuanceFrameworkLoaded = safeHandler(function nuanceFrameworkLoaded(): void {
  ...
  w.nuanceReactive_HMRC_CIAPI_Fixed_1 = safeHandler(
    function nuanceReactive_HMRC_CIAPI_Fixed_1(
      c2cObj: ClickToChatObjectInterface,
    ): void {
  ...
  w.nuanceRestoreReactive = safeHandler(function nuanceRestoreReactive(): void {
    logger.debug("### nuanceRestoreReactive");

    if (
      event.rule &&
      event.rule["name"] === "HMRC-C-LC-CIAPI-TES-O-R-DTS-Anchored-C2C" &&
      event.evtType === "SHOWN" &&
      window.location.href.includes("/personal-account")
    ) {
      window.Inq.SDK.closeChat();
    } else {
